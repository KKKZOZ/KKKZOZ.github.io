...
> [!summary] Sliding Window Technique
>
> LLM 在逐字生成文本时，相邻 token 所激活的神经元集合通常有很高的重叠度。例如，在生成序列“... a cute cat”时，“cat”这个词激活的神经元集合，有很大一部分可能与“cute”激活的神经元集合是相同的。
>
> 滑动窗口技术正是利用了这一特性。它不是在生成每个新 token 时都将 DRAM 清空，然后从闪存加载全新的神经元权重，而是在 DRAM 中维护一个缓存区，保留最近 k 个 token 所激活过的所有神经元的权重
...
为了避免在 DRAM 中频繁地重新分配内存和复制数据，论文采用了一种优化的内存管理策略。系统会预先分配一块足够大的内存区域，并使用一个包含指针、矩阵、偏置等元素的数据结构来高效地管理神经元的删除和插入操作。
