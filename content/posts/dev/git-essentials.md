...
| 模式 | 命令 (`<commit>` 是目标提交，如 `HEAD~1` 或某个 commit hash) | 提交历史 (HEAD) | 暂存区 (Index) | 工作目录 | 常用场景 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **`--soft`** | `git reset --soft <commit>` | **移动** | **不变** | **不变** | 合并多个提交 (Squash) |
| **`--mixed`** | `git reset --mixed <commit>` 或 `git reset <commit>` | **移动** | **重置** | **不变** | 修改最后一次提交的内容、取消暂存 |
| **`--hard`** | `git reset --hard <commit>` | **移动** | **重置** | **重置** | **彻底**放弃最近的提交和修改 |
...
- **`git reset --mixed <commit>` (默认模式)**
  - 移动 `HEAD` 指针，并且**清空（重置）暂存区**，使其内容与指定的 `<commit>` 保持一致。
  - 工作目录的文件内容不会被改变。
...
- **`git reset --hard <commit>`**
  ...
  - 所有在 `<commit>` 之后的所有提交以及工作目录中未提交的本地修改都将**永久丢失**（除非使用 `git reflog` 等高级技巧尝试恢复）。
